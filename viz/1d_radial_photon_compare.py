...
for i in range(time_slices):
        ...
        plt.subplot(1,1,1)
        plt.plot(y,Z_bourdon[:,int(x_size/2)], label="bourdon")
        plt.plot(y,Z_fft[:,int(x_size/2)], label="fft")
        plt.ylim(15,30)
        plt.legend()

        plt.savefig("../images/photon_radial_out_" + str(c_time_slice) + ".png", dpi=200)
        plt.clf()
        plt.close()
