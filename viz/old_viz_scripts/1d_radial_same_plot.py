...
for i in range(time_slices):
	...
	plt.subplot(2,3,1)
	plt.plot(y,Z_potential[:,int(pos)],linewidth=1)
	#plt.ylim(-6000,10000)

	plt.subplot(2,3,2)
	plt.plot(y,Z_field_x[:,int(pos)],linewidth=1)
	#plt.ylim(-1e7,1e7)

	plt.subplot(2,3,3)
	plt.plot(y,Z_field_y[:,int(pos)],linewidth=1)
	#plt.ylim(-10000,10000)

	plt.subplot(2,3,4)
	plt.plot(y,Z_electron_density[:,int(pos)],linewidth=1)
	#plt.ylim(10,20)
	#plt.contourf(X_g,Y_g,Z_electron_density, levels=num_levels, extend='both', cmap="plasma")

	plt.subplot(2,3,5)
	plt.plot(y,Z_ion_density[:,int(pos)], linewidth=1)
	#plt.ylim(10,20)
	#plt.contourf(X_g,Y_g,Z_ion_density, levels=num_levels, extend='both', cmap="plasma")

	plt.subplot(2,3,6)
	plt.plot(y,Z_total_density[:,int(pos)], linewidth=1)
	#plt.ylim(10,20)
	#plt.contourf(X_g, Y_g, volumetric_term, extend='both')
	#infile = open("data/data_out_" + str(n) + ".dat","r");
	#plt.clf()
	#plt.close()
plt.savefig("../images_1d/same_plot.png", dpi=200)
