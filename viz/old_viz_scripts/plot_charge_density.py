import math
import matplotlib
matplotlib.use('agg')
import matplotlib.pyplot as plt
import numpy as np
from matplotlib import ticker

num_procs = 2
x_size = 0
y_size = 0
space_step = 0
skip = 1
time_slices = 200
file_begin = 0
file_stride = 5000
constant_file = open("../constants.h","r")
for line in constant_file:
	line = line.strip().split()
	
	if(len(line) > 0 and len(line) == 3):
		if(line[1] == "XSIZE"):
			x_size = int(line[2]);
		if(line[1] == "YSIZE"):
			y_size = int(line[2]);
		if(line[1] == "SPACE_STEP"):
			space_step = float(line[2]);
	
print("x_size = " + str(x_size))
print("y_size = " + str(y_size))
print("space_step = " + str(space_step))
constant_file.close()

x = np.linspace(0,(x_size-1)*space_step,x_size)
y = np.linspace(0,(y_size-1)*space_step,y_size)
Z_cd = np.ones((y_size,x_size))

X_g, Y_g = np.meshgrid(x, y)
X_g = X_g / 1e-3
Y_g = Y_g / 1e-3

dir_i = "/users/asfierro/wheeler-scratch/2d_fluid_axisymmetric/data/"
for i in range(time_slices):
	c_time_slice = i*file_stride + file_begin;
	proc_y_size = int(y_size / num_procs)
	y_size_add = y_size % num_procs

	cur_y_pos = 0
	for n in range(num_procs):
		if n == (num_procs-1):
			y_size = y_size + y_size_add
		infile = open(dir_i + "mesh_data/charge_density_out_" + str(c_time_slice) + "_" + str(n) + ".csv","r");
		#infile = open("data/data_out_" + str(n) + "_" + str(c_time_slice) + ".dat","r");
		print("opening file: " + str(n) + " with time plane = " + str(c_time_slice))
		cur_x_pos = 0
		infile.readline()
		for line in infile:
			line = line.strip().split(',')

			Z_cd[cur_y_pos][cur_x_pos] = float(line[3])
		
			cur_x_pos = cur_x_pos + 1
			if cur_x_pos == x_size:
				cur_x_pos = 0
				cur_y_pos = cur_y_pos + 1

		infile.close();
	cur_y_pos = 0

	num_levels = np.linspace(0,25000,100)
	plt.figure(figsize=(12,12.0*(y_size/x_size)), dpi=500)
	plt.subplot(1,1,1)
	plt.contourf(X_g,Y_g,Z_cd,extend='both')
	#plt.ylim(-2,2)
	plt.grid()
	plt.colorbar()

	#infile = open("data/data_out_" + str(n) + ".dat","r");
	plt.savefig("../images/charge_density_img_out_" + str(c_time_slice) + ".png", dpi=200)
	plt.clf()
	plt.close()

