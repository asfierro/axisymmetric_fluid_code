...
y_size = 1024
space_step = 50e-6
dir_i = "/users/asfierro/carc-scratch/air_streamer_5um_no_photons/"
...
for i in range(time_slices):
        ...
        for n in range(num_procs):
                ...
                infile.close()
        '''
        num_levels = np.linspace(-20,40,25)
        plt.figure(figsize=(12,12.0*(y_size/x_size)), dpi=500)
        plt.subplot(2,4,1)
        plt.contourf(X_g,Y_g,Z_potential,extend='both')
        plt.contourf(X_g,Y_g*-1,Z_potential,extend='both')
        #plt.ylim(-2,2)
        plt.colorbar()

        num_levels = np.linspace(-2e5,2e5,20)
        plt.subplot(2,4,2)
        ...
        num_levels = np.linspace(0,5e6,100)
        plt.subplot(2,4,3)
        plt.contourf(X_g,Y_g,Z_field_y, extend='both')
        plt.contourf(X_g,Y_g*-1,Z_field_y,extend='both')
        plt.grid()
        #plt.ylim(-2,2)
        plt.colorbar()

        num_levels = np.linspace(0,2e7,20)
        plt.subplot(2,4,4)
        plt.contourf(X_g,Y_g,Z_field_mag,levels=num_levels,extend='both', cmap="rainbow")
        plt.contourf(X_g,Y_g*-1,Z_field_mag,levels=num_levels,extend='both', cmap="rainbow")
        #plt.ylim(-2,2)
        plt.colorbar()

        #num_levels = np.linspace(1.0e8, 1.0e19, 50)
        #num_levels = [1e8, 1e9, 1e10, 1e11, 1e12, 1e13, 1e14, 1e15, 1e16, 1e17, 1e18]
        num_levels = np.linspace(10,17,100)
        plt.subplot(2,4,5)
        plt.contourf(X_g,Y_g,Z_cd, extend='both', cmap="rainbow")
        plt.contourf(X_g,Y_g*-1,Z_cd, extend='both', cmap="rainbow")
        #plt.ylim(-2,2)
        plt.colorbar()

        plt.subplot(2,4,6)
        num_levels = np.linspace(18,28,8)
        plt.contourf(X_g, Y_g, volumetric_term, levels=num_levels,extend='both')
        plt.contourf(X_g, Y_g*-1, volumetric_term, levels=num_levels,extend='both')
        #plt.ylim(-2,2)
        plt.colorbar()
